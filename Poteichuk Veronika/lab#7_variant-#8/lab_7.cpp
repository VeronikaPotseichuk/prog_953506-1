/*Супермаркет. Программа для кассового аппарата. 
Имеется база уникальных штрих-кодов (представлены в виде числовых значений). 
С каждым штрих-кодом связано название товара и его цена. 
При вводе задается количество нужного товара (ничего не введено – 1 штука, введен 0 – отмена ввода). 
Вести историю всех покупок. 
После окончания ввода всех товаров одного покупателя программа спрашивает, есть ли дисконтная карта, и если есть, то требует ее серийный номер. 
Сверяет по базе дисконтных карт, берет оттуда процент скидки, производит расчет и записывает эту информацию в историю покупки. */
...
int ReadBarCode()
{
	...
	do
	{
		...
		while (i < 6 && (c = _getch()) != 13) //Проверка на 6-значность и получение символа с клавиатуры
		{
			str[i++] = c;
			_putch(c);
		}
	...
	} while (correct != true); //Если число не 6-значное - ввод числа повторяется
